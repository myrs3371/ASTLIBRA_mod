...
### 制作mod
1. 将修改后的文件放入 `MODS/` 文件夹中，结构参考上一步
2. 创建 `mod_info.json` 中填写相关信息
3. 开始使用
...
**Q: JSON 文件有什么用？**
A: MOD 文件夹中的 `mod_info.json`（可选）用于存储 MOD 的名称、版本、作者和描述信息；`MODS/active_mods.json`（自动生成）记录当前已激活的 MOD 列表，激活时会自动创建和更新。
